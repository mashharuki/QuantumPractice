...
// switch drawing off to increase sim speed
var increase_speed = false; 

// ショアの素因数分解アルゴリズムを実行する関数
function shor_sample() {
    // 素因数分解する関数
    var N = 35;             
    // 精度レジスタのビット数(comprimeに乗ずる数)
...
function Shor(N, precision_bits, coprime) {
    // QPUによる演算セクション(反復周期を見つける)
    var repeat_period = ShorQPU(N, precision_bits, coprime);
     // 従来のコンピュータによる演算セクション (素因数を見つける)
